...
def collect_honey():
    ...
    while True:
        ...
        last_buying_datetime, last_buying_price = get_last_transaction_price('buy')
        last_selling_datetime, last_selling_price = get_last_transaction_price('sell')
        action = determine_status(available_eur, available_eth, current_stock_price, RESERVE, FEE, BUYING_MARGIN, SELLING_MARGIN, past_stock_prices_1d,past_stock_prices_7d_1d, past_stock_prices_10m,last_selling_datetime, last_selling_price,last_buying_datetime, last_buying_price)
        print("action", action)
        if action == 'buy':
